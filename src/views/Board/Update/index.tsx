...
export default function BoardUpdate() {
  ...
  const getBoardResponse = (responseBody: GetBoardResponseDto | ResponseDto | null) => {
    ...
    const { title, content, boardImageList, wrtierEmail } = responseBody as GetBoardResponseDto;
    setTitle(title);
    setContent(content);
    setImageUrls(boardImageList);
    convertUrlsToFile(boardImageList).then(boardImageFileList => setBoardImageFileList(boardImageFileList));

    if (!loginUser || loginUser.email != wrtierEmail) {
      navigate(MAIN_PATH());
      return;
    }
      ...
      <div className='board-update-container'>
        <div className='board-update-box'>
          <div className='board-update-title-box'>
            <textarea ref={titleRef} className='board-update-title-textarea' rows={1} placeholder='제목을 작성해주세요.' value={title} onChange={onTitleChangeHanlder} />
          </div>
